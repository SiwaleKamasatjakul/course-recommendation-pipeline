...
```
Events are generated by a local web app running at `http://localhost:3000`.
Browsing activities, expanding a competition's details, or dwelling on a page
emits a JSON event straight to the `clickstream.events` topic — so the whole
pipeline can be exercised by clicking through the UI rather than running a
synthetic producer.
